...
> **本文件是工程根的统一入口。** 任何 AI 开发工具（Codex / WorkBuddy / Claude Code / Cursor …）
> 打开本目录时都应先读本文件，再动手。
...
5. **不要触碰**：`C:\KEY.json`、`C:\KEY - 副本.json`（与本项目无关的本地凭据）。
...
- 项目知识以 `docs/` + `交接说明.md` + 本文件为准（`.workbuddy/` 只是某个工具的私有记忆，不入库）。
