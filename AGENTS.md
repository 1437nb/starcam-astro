...
- 当前基线：**v1.5.52**（versionCode 72）；v1.5.49 与 v1.5.51 已发布到 GitHub Releases。
  v1.5.51（§0.62 宽场打分轮）发版后用户报告"离线演示 12 张照片超 1 分钟未识别"，
  已定位为打分轮在失败路径的额外开销（+15.5s）并在 v1.5.52 修复（+2.5~3s），
  v1.5.52 待真机验证后发版。
