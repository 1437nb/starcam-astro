 ...
 * （视场 10°~120°，画面含 ≥8 颗可检测亮星）。
 *
 * §0.75 起采用**分级星表域**：浅域（mag≤4.0，514 颗）承担 30° 以上的宽场，
 * 整条兜底阶梯都无解时才切到深域（mag≤6.5，8415 颗）重试一次，把能力下沿
 * 从 ~20° 拉到 10°。宽场照片在浅域就解出，永远碰不到深域 —— 这是分级而非
 * 全局加深的原因：实测全局加深会让宽场弱解被稀释、并引入尺度坍缩伪影。
 */
