...
- **v1.5.61 修复「SEP 提星阈值语义误用」（§0.72，用户报告）**：
  ...
  `plim`，而 plim 是峰值显著度、检出限 `limit = sigma/(2√π·dpsf) × plim` ——
  plim=2.0 实际只相当于 **0.56σ**。实测用户照片：过阈像素 29.27% → dmask 连成
  **848,846 px 巨块** → 超 maxsize=2000 被判「延展天体」整体丢弃 → 真星全丢，
  report.json 里 172 颗"星"0 颗落在亮源上、73% 落在 30px 边界带（富集 11.6 倍）。
  这解释了用户观察到的「高对比度版能识别、原图不能」。
  修复：C 层换算 plim = N×2√π×dpsf（2σ→7.09）+ 下限 4.0；边界过滤 16px；
  提星质量核验（与 box-blur 重合率 <30% 回退）+ **提星诊断写入识别日志**。
  验证：过阈像素 29.27%→0.63%、最大连通域 848,846→55px、单测 170/170。
  详见 `docs/70-…§0.72…`。
  v1.5.49/51/52/53/55/57/58/59 已发布，v1.5.54/56 未单独发布（内容并入后续版本）。
- **v1.5.60 修复「2026-09-17 照片识别失败 + 旧照片回归」（§0.71，用户报告）**：
  四层缺陷叠加 —— ①投票阈值量纲错配（900/60/25 绝对值 vs SEP flux 量纲 → work=0 空转；
  §0.64 只补了判据没补阈值）；②打分轮 y 镜像写死（真实照片 parity 两种都有，写死
  单向必错一类 → 旧照回归根因；改为双 parity 拟合取残差小者，分离 300 倍）；
  ③打分轮胜出模型的投影中心与展开不一致（24 内点解作废 → 展开前补原点迭代）；
  ④赤经跨 0° 接缝用算术平均（345.5°/2.1°/3.3° 平均出 117°，tanXY 全 NaN →
  改球面向量平均）。
  另修**跨进程结果随机翻转**（triKey 未规范化 HIP 顺序 → 同三角形重复记录在并列处
  按 HashMap 遍历序截断；规范化 + 排序加 hip 决胜 → 4/4 次全量回归逐位一致）。
  验证：新照/旧照各渲染 inl=25、gray12 12/12、窄场 apod4 SOLVED、假阳性 8 张全拒、
  单测 165/165。详见 `docs/69-…§0.71…`。
- v1.5.59 为「识别日志：失败原因与现场留存」（§0.70）：`SolveLogStore`（release 也生效）
  滚动日志 + 失败现场（`report.json` + `input.gray.gz`，gzip 后 14.5% 体积），
  三个引擎分支全路径归因，失败页可折叠技术详情 + 一键复制。详见 `docs/68-…§0.70…`。
